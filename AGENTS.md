...
Current implementation: `components/GoogleAnalytics.tsx` stores the visitor's analytics choice in local storage, queues the initial GA4 page view immediately after consent, then dynamically loads `gtag.js` so the queue always exists before Google processes it. It tracks subsequent App Router pathname changes. The measurement ID is supplied through `NEXT_PUBLIC_GA_MEASUREMENT_ID`.
